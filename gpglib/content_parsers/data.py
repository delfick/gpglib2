...
class SymmetricallyEncryptedIntegrityProtectedDataPacketParser(Parser):
    ...
    def consume(self, tag, message, region, algo, session_key):
        ...
        cipher = cipherkls.new(
            session_key, cipherkls.MODE_CFB, b"\x00" * block_size, segment_size=segment_size
        )
        ...
        if not hasattr(message, "mdc"):
            raise errors.PGPException(
                "Integrity protected message is missing modification detection code."
            )
        ...
        if message.mdc != hash_.digest():
            raise errors.PGPException(
                "Integrity protected message does not match modification detection code."
            )
